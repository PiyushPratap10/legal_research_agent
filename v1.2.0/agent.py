...
async def legal_agent(query,sid):
    ...
    agent = FunctionAgent(
        tools=[get_constitution_response,get_criminal_response,get_civil_response,web_search_tool],
        llm=Settings.llm,
        verbose=True,
        system_prompt=f"""You are a highly specialized Legal Research Agent for Indian law. 
You have access to four expert tools: Constitution Research, Criminal Law Research, Civil Law Research, and Web Legal Search. 
Your job is to carefully choose the correct tool and generate reliable, well-cited answers. You also have the access of conversation history to maintain context.
...
- Use Criminal Law Research ONLY for queries about BNS, BNSS, BSA, IPC, CrPC, punishments for crimes, criminal offenses, rights of accused, bail, investigation, or trial procedures.
...
- Use Web Legal Search ONLY when:
    ...
    2. Indexed Constitution/Criminal/Civil resources do not provide any relevant context.
    3. The query requires up-to-date information (latest amendments, judgments, news).
- If the user query refers to previous conversation or chats, use the Conversation History to answer the user query.
...
"""
    ...
    conversation_memory[sid] = history
    return str(response)
